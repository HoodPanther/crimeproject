...
if __name__ == "__main__":
    ...
    results = lines.map(lambda x: (date(int(x[1][6:10]), int(x[1][0:2]), int(x[1][3:5])).weekday(), 1) if len(x[1])==10 else ('-1', 1) ) \
                   .reduceByKey(add) \
                   .sortBy(lambda x: x[0]) \
                   .map(lambda x: str(x[0]) + '\t' + str(x[1])) \

    results.saveAsTextFile('all_offense_day_of_week.out')
